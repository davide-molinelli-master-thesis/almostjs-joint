...
function Board(options) {
    ...
    self.zoomVoiceAssistant = function (times,delta) {
        zoomReactor.zoomVoiceAssistant(times,delta);
    };

    self.moveBoardVoiceAssistant = function (times,delta) {
        var p = paper.snapToGrid({x: Math.floor(window.innerWidth/2), y: Math.floor(window.innerHeight/2)});
        var panReactor = createPanReactor({paper: paper, el: el, x: p.x, y: p.y, simulation: true});

        panReactor.moveBoardVoiceAssistant(times,delta);
    };

    self.dragDropElementVoiceAssistant = function (cell, delta) {
        embeddingReactor.dragDropElementVoiceAssistant(cell,delta);
    }

    self.resizeElementVoiceAssistant = function (cell, delta, direction) {
        var cellView = paper.findViewByModel(cell);
        var resizeReactor = createResizeReactor({view: cellView, paper: paper, model: model, el: el, direction: direction, gridSize: gridSize, undoReactor: undoReactor});
        resizeReactor.resizeElementVoiceAssistant(cell,delta,direction);
    }

    self.generateElementVoiceAssistant = function (cell) {
          embeddingReactor.generateElementVoiceAssistant(cell);
    }
