...
function EmbeddingReactor(options) {
    ...
    self.dragDropElementVoiceAssistant = function (cellView, delta) {
        dragDropElementSimulator(cellView, delta);
    }

    async function dragDropElementSimulator (cellView,delta) {
        ...
        for (var i = 0; i < times; i++) {
            var x = delta.x !== 0 ? cellView.position().x + xDelta : cellView.position().x,
                y = delta.y !== 0 ? cellView.position().y + yDelta : cellView.position().y;

            cellView.position(x,y);
