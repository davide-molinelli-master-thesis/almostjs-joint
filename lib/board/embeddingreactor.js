...
function EmbeddingReactor(options) {
    ...
    function dragElement (cellView, evt, x, y) {
        _.noop(evt, x, y);
    ...
    function dropElement (cellView, evt, x, y) {
        _.noop(evt, x, y);
        var element = cellView.model,
            newPosition,
            oldparent,
    ...
    function addElement (cell) {
        ...
        element.set('z', model.maxZIndex() + 1);
    }

    self.dragDropElementVoiceAssistant = function (cell, delta) {
        dragDropElementSimulator(cell, delta);
    }

    async function dragDropElementSimulator (cell,delta) {
        var times = Math.max(Math.abs(delta.x),Math.abs(delta.y)),
            xDelta = 0,
            yDelta = 0,
            cellView = paper.findViewByModel(cell);

        dragElement(cellView);
        ...
        for (var i = 0; i < times; i++) {
            var x = delta.x !== 0 ? cell.position().x + xDelta : cell.position().x,
                y = delta.y !== 0 ? cell.position().y + yDelta : cell.position().y;
    ...
    paper.on('cell:pointerdown', dragElement);
    paper.on('cell:pointerup', dropElement);

    model.on('add', addElement);
}
