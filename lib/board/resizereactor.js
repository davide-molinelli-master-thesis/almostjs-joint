...
function ResizeReactor(options) {
    ...
    function mousemove(e) {
        var client = e.simulation ? constrain({x: e.clientX, y: e.clientY}, direction) : constrain(fixPoint(paper.clientToLocalPoint({x: e.clientX || e.x, y: e.clientY || e.y}), direction)),
    ...
    self.resizeElementVoiceAssistant = function (cell, delta,direction) {
        resizeElementSimulator(cell,delta,direction);
    }

    async function resizeElementSimulator (cell,delta,direction) {
        var times = Math.abs(delta) / 10,
            xDelta = 0,
            yDelta = 0,
            eventSimulator = {
              simulation: true
        ...
        try {
            switch (direction) {
              case 'n':
                eventSimulator.clientX = cell.position().x + cell.size().width/2;
                eventSimulator.clientY = cell.position().y;
                if (delta > 0) {
                    yDelta = -10;
                } else if (delta < 0) {
                    yDelta = 10;
              ...
              case 's':
                eventSimulator.clientX = cell.position().x + cell.size().width/2;
                eventSimulator.clientY = cell.position().y + cell.size().height;
                if (delta > 0) {
                    yDelta = 10;
                } else if (delta < 0) {
                    yDelta = -10;
                ...
                break;
              case 'e':
                eventSimulator.clientX = cell.position().x + cell.size().width;
                eventSimulator.clientY = cell.position().y + cell.size().height/2;
                if (delta > 0) {
                    xDelta = 10;
                } else if (delta < 0) {
                    xDelta = -10;
                ...
                break;
              case 'w':
                eventSimulator.clientX = cell.position().x;
                eventSimulator.clientY = cell.position().y + cell.size().height/2;
                if (delta > 0) {
                    xDelta = -10;
                } else if (delta < 0) {
                ...
                break;
              default:
                throw 'Unexpected resize direction request';
                break;
            }
        } catch (exception) {
        ...
        for (var i = 0; i < times; i++) {
            eventSimulator.clientX += xDelta;
            eventSimulator.clientY += yDelta;
