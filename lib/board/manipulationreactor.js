...
function ManipulationReactor(options) {
    ...
    function toggleManipulator(cellView, evt) {
        ...
        if (!bundle) {
            ...
            if(bundle.view.model.attributes.pattern){
                if(_.filter(bundle.view.model.attributes.pattern, function (p) {return p.type === 'root' && !p.active}).length > 0) {
                  (function () {
                      var pattern = $(require('./connections.svg')),
                          trigger = function () {
                            self.trigger('cell:pattern-brain', bundle.view);
                          };
                      tab.append($(pattern).clone().on('click', trigger));
                  }());
                } else if (_.filter(bundle.view.model.attributes.pattern, function (p) {return p.type === 'root' && p.active}).length > 0) {
                  (function () {
                      var pattern = $(require('./pattern.svg')),
                          trigger = function () {
                            self.trigger('cell:pattern-load', bundle.view);
                          };
                      tab.append($(pattern).clone().on('click', trigger));
                  }());
                }
                    ...
                    if (action.click) {
                        (function () {
                            function click() {
                                ...
                                if(action.shortcut === 'del' && cellView.model.attributes.pattern){
                                  self.trigger('cell:pattern-check', cellView);
                                }
                            }
                            tab.append($(action.marker).clone().on('click', click));
                            if (action.popover){
                                action.popover.call(bundle.context, bundle.view);
                            }
                        }());
                    }
                    if (action.event) {
...
exports.delete = {
    ...
    popover: function (view) {
        if(view.model.attributes.pattern && view.model.attributes.pattern.active){
              $('#delete-icon').popover({
                title: "Warning",
                content: "This operation could compromise irreparably the pattern",
                placement: "auto top",
                trigger: "hover"
              });
        };
